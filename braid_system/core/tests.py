...
def criar_atendimento(estabelecimento, cliente=None, **extra):
    if cliente is None:
        cliente = Cliente.objects.create(estabelecimento=estabelecimento, apelido='Cliente X')
...
class UsuarioModelTests(TestCase):
    ...
    def test_create_user_sem_email_levanta_erro(self):
        with self.assertRaises(ValueError):
            Usuario.objects.create_user(email='', nome='SemEmail', password='x123456789')

    def test_create_user_normaliza_dominio_email(self):
        u = Usuario.objects.create_user(email='Pessoa@EXEMPLO.COM', nome='P', password='x123456789')
        self.assertEqual(u.email, 'Pessoa@exemplo.com')

    def test_create_superuser(self):
        su = Usuario.objects.create_superuser(email='s@b.com', nome='Sup', password='x123456789')
...
class ModelTests(TestCase):
    ...
    def test_pagamento_str_sem_forma(self):
        ...
        pag = Pagamento.objects.create(atendimento=at, forma_pagamento=None, valor=Decimal('150'))
    ...
    def test_pagamento_str_com_forma(self):
        ...
        pag = Pagamento.objects.create(atendimento=at, forma_pagamento=forma, valor=Decimal('150'))
    ...
    def test_custo_str(self):
        ...
        custo = Custo.objects.create(
            estabelecimento=est, categoria_custo=cat,
            descricao='Junho', data=date(2026, 6, 1), valor=Decimal('800'),
        )
        self.assertIn('Aluguel', str(custo))
        self.assertIn('800', str(custo))

    def test_caracteristica_ordenacao_e_defaults(self):
        c2 = CaracteristicaAtendimento.objects.create(ordem=2, nome='Tamanho', pergunta='Qual?')
        c1 = CaracteristicaAtendimento.objects.create(ordem=1, nome='Tipo', pergunta='Qual?')
        self.assertEqual(list(CaracteristicaAtendimento.objects.all()), [c1, c2])  # ordering
    ...
    def test_opcao_str_e_hierarquia(self):
        carac = CaracteristicaAtendimento.objects.create(ordem=1, nome='Tipo', pergunta='Qual?')
        pai = CaracteristicaAtendimentoOpcao.objects.create(caracteristica_atendimento=carac, nome='Box braids')
    ...
    def test_atendimento_caracteristica_unico(self):
        ...
        at = criar_atendimento(est)
        carac = CaracteristicaAtendimento.objects.create(ordem=1, nome='Tipo', pergunta='Qual?')
        opcao = CaracteristicaAtendimentoOpcao.objects.create(caracteristica_atendimento=carac, nome='Box')
...
class ModelDeletionRulesTests(TestCase):
    ...
    def test_atendimento_cascateia_pagamento_e_caracteristica(self):
        est = criar_estabelecimento()
        at = criar_atendimento(est)
        Pagamento.objects.create(atendimento=at, forma_pagamento=None, valor=Decimal('10'))
        carac = CaracteristicaAtendimento.objects.create(ordem=1, nome='Tipo', pergunta='Q')
        opcao = CaracteristicaAtendimentoOpcao.objects.create(caracteristica_atendimento=carac, nome='Box')
        AtendimentoCaracteristica.objects.create(atendimento=at, opcao=opcao)
        at.delete()
...
class ContextProcessorTests(TestCase):
    ...
    def test_auto_selecao_quando_unico_vinculo(self):
        ...
        self.assertEqual(req.session['estabelecimento_ativo_id'], str(est.pk))  # persistiu

    def test_multiplos_vinculos_sem_selecao_retorna_none(self):
        user = criar_usuario()
        for nome in ('A', 'B'):
            EstabelecimentoUsuario.objects.create(estabelecimento=criar_estabelecimento(nome), usuario=user)
...
class AuthFlowTests(TestCase):
    ...
    def test_login_invalido(self):
        resp = self.client.post(reverse('login'),
                                {'username': self.user.email, 'password': 'errada'})
...
class EstabelecimentoViewTests(AdminLogadoMixin, TestCase):
    ...
    def test_criar_valido(self):
        resp = self.client.post(reverse('estabelecimento_criar'), {'nome': 'Novo Salao'})
    ...
    def test_editar(self):
        ...
        resp = self.client.post(reverse('estabelecimento_editar', args=[est.pk]), {'nome': 'Renovado'})
        self.assertRedirects(resp, reverse('estabelecimentos'))
        est.refresh_from_db()
    ...
    def test_pagina_publica_cadastro(self):
        self.assertEqual(self.client.get(reverse('cadastro_estabelecimento')).status_code, 200)
        resp = self.client.post(reverse('cadastro_estabelecimento'), {'nome': 'Via Cadastro'})
...
class CategoriaCustoViewTests(AdminLogadoMixin, TestCase):
    ...
    def test_criar_com_flag_vinculado(self):
        resp = self.client.post(reverse('categoria_custo_criar'),
    ...
    def test_editar(self):
        ...
        resp = self.client.post(reverse('categoria_custo_editar', args=[cat.pk]), {'nome': 'Novo'})
        self.assertRedirects(resp, reverse('categorias_custo'))
        cat.refresh_from_db()
...
class CaracteristicaViewTests(AdminLogadoMixin, TestCase):
    def test_criar_valido(self):
        resp = self.client.post(reverse('caracteristica_atendimento_criar'),
                                {'nome': 'Tipo', 'pergunta': 'Qual tipo?', 'ordem': '1',
                                 'numero_maximo_selecao': '2', 'contem_dado_sensivel': 'on'})
    ...
    def test_editar(self):
        ...
        resp = self.client.post(reverse('caracteristica_atendimento_editar', args=[c.pk]),
    ...
    def test_excluir(self):
        c = CaracteristicaAtendimento.objects.create(ordem=1, nome='A', pergunta='Q')
        resp = self.client.post(reverse('caracteristica_atendimento_excluir', args=[c.pk]))
        self.assertRedirects(resp, reverse('caracteristicas_atendimento'))
        self.assertEqual(CaracteristicaAtendimento.objects.count(), 0)
    ...
    def test_opcoes_crud(self):
        ...
        # listar
        self.assertEqual(
            self.client.get(reverse('caracteristica_atendimento_opcoes', args=[c.pk])).status_code, 200)
        # criar
        resp = self.client.post(reverse('opcao_caracteristica_criar', args=[c.pk]),
                                {'nome': 'Box braids'})
        self.assertRedirects(resp, reverse('caracteristica_atendimento_opcoes', args=[c.pk]))
        opcao = CaracteristicaAtendimentoOpcao.objects.get(nome='Box braids')
        # criar subopcao
        self.client.post(reverse('opcao_caracteristica_criar', args=[c.pk]),
                         {'nome': 'Fina', 'nivel_superior': str(opcao.pk)})
        self.assertEqual(CaracteristicaAtendimentoOpcao.objects.get(nome='Fina').nivel_superior, opcao)
        # editar
        self.client.post(reverse('opcao_caracteristica_editar', args=[c.pk, opcao.pk]),
                         {'nome': 'Box braids G'})
        opcao.refresh_from_db()
        self.assertEqual(opcao.nome, 'Box braids G')
        # excluir
        resp = self.client.post(reverse('opcao_caracteristica_excluir', args=[c.pk, opcao.pk]))
        self.assertRedirects(resp, reverse('caracteristica_atendimento_opcoes', args=[c.pk]))
...
class UsuarioViewTests(AdminLogadoMixin, TestCase):
    ...
    def test_criar_valido(self):
        resp = self.client.post(reverse('usuario_criar'),
                                {'nome': 'Nova', 'email': 'nova@b.com',
                                 'tipo': 'profissional', 'password': 'senha123456'})
    ...
    def test_editar_desativa(self):
        ...
        resp = self.client.post(reverse('usuario_editar', args=[u.pk]),
...
class AcessoViewTests(TestCase):
    ...
    def test_listar(self):
        self.assertEqual(self.client.get(reverse('acessos_estabelecimento')).status_code, 200)

    def test_criar_registra_incluido_por(self):
        resp = self.client.post(reverse('acesso_criar'),
                                {'usuario': str(self.alvo.pk), 'estabelecimento': str(self.est.pk),
                                 'tipo_acesso': 'editar'})
        self.assertRedirects(resp, reverse('acessos_estabelecimento'))
        vinc = EstabelecimentoUsuario.objects.get(usuario=self.alvo, estabelecimento=self.est)
        self.assertEqual(vinc.tipo_acesso, 'editar')
        self.assertEqual(vinc.incluido_por, self.admin)

    def test_criar_duplicado(self):
        EstabelecimentoUsuario.objects.create(usuario=self.alvo, estabelecimento=self.est)
        resp = self.client.post(reverse('acesso_criar'),
                                {'usuario': str(self.alvo.pk), 'estabelecimento': str(self.est.pk),
                                 'tipo_acesso': 'ver'})
        self.assertEqual(resp.status_code, 200)
    ...
    def test_editar_tipo_acesso(self):
        ...
        resp = self.client.post(reverse('acesso_editar', args=[vinc.pk]), {'tipo_acesso': 'administrar'})
        self.assertRedirects(resp, reverse('acessos_estabelecimento'))
        vinc.refresh_from_db()
        self.assertEqual(vinc.tipo_acesso, 'administrar')

    def test_excluir(self):
        vinc = EstabelecimentoUsuario.objects.create(usuario=self.alvo, estabelecimento=self.est)
    ...
    def test_filtro_por_estabelecimento(self):
        ...
        resp = self.client.get(reverse('acessos_estabelecimento'), {'estabelecimento': str(self.est.pk)})
...
class ClienteViewTests(AutenticadoComEstabelecimentoMixin, TestCase):
    ...
    def test_criar(self):
        resp = self.client.post(reverse('cliente_criar'),
                                {'apelido': 'Dona Rita', 'descricao': 'Cliente fiel'})
        self.assertRedirects(resp, reverse('clientes'))
        self.assertTrue(Cliente.objects.filter(apelido='Dona Rita', estabelecimento=self.est).exists())
    ...
    def test_editar(self):
        ...
        resp = self.client.post(reverse('cliente_editar', args=[c.pk]), {'apelido': 'Novo'})
        self.assertRedirects(resp, reverse('clientes'))
        c.refresh_from_db()
...
class AtendimentoViewTests(AutenticadoComEstabelecimentoMixin, TestCase):
    ...
    def test_get_criar_redireciona(self):
        ...
        self.assertRedirects(self.client.get(reverse('atendimento_criar')), reverse('atendimentos'))

    def test_criar_com_novo_cliente_e_pagamento(self):
        """Regressao: antes quebrava por forma_pagamento='' numa FK."""
        resp = self.client.post(reverse('atendimento_criar'), {
            'novo_cliente': 'Maria',
            'data': '2026-06-01',
            'hora': '14:30',
            'duracao': '02:00',
    ...
    def test_criar_com_caracteristicas_e_custos(self):
        ...
        resp = self.client.post(reverse('atendimento_criar'), {
            ...
            'data': '2026-06-02', 'hora': '09:00', 'pagamento_valor': '200',
            'opcoes': [str(opcao.pk)],
    ...
    def test_criar_pagamento_invalido_falha(self):
        resp = self.client.post(reverse('atendimento_criar'), {
            'novo_cliente': 'Maria', 'data': '2026-06-01', 'hora': '14:30',
            'pagamento_valor': 'abc'})
        self.assertRedirects(resp, reverse('atendimentos'))
        self.assertEqual(Atendimento.objects.count(), 0)
    ...
    def test_criar_sem_estabelecimento_ativo(self):
        ...
        sessao.save()
        resp = self.client.post(reverse('atendimento_criar'), {
            'novo_cliente': 'Maria', 'data': '2026-06-01', 'hora': '14:30', 'pagamento_valor': '150'})
        self.assertRedirects(resp, reverse('atendimentos'))
        self.assertEqual(Atendimento.objects.count(), 0)

    def test_editar_atualiza_pagamento(self):
        at = criar_atendimento(self.est)
        Pagamento.objects.create(atendimento=at, forma_pagamento=None, valor=Decimal('100'))
        resp = self.client.post(reverse('atendimento_editar', args=[at.pk]), {
            'data': '2026-07-01', 'hora': '10:00', 'duracao': '01:15', 'pagamento_valor': '180'})
        self.assertRedirects(resp, reverse('atendimentos'))
        at.refresh_from_db()
...
class CustoViewTests(AutenticadoComEstabelecimentoMixin, TestCase):
    def setUp(self):
        super().setUp()
        self.cat = CategoriaCusto.objects.create(nome='Aluguel', vinculado_atendimento=False)
    ...
    def test_listar_mes_vazio_nao_quebra(self):
        ...
        self.assertEqual(self.client.get(reverse('custos'), {'mes': '', 'ano': ''}).status_code, 200)

    def test_criar(self):
        resp = self.client.post(reverse('custo_criar'), {
            'categoria_custo': str(self.cat.pk), 'descricao': 'Junho',
            'data': '2026-06-10', 'valor': '800,00', 'mes': '6', 'ano': '2026'})
    ...
    def test_criar_categoria_vinculada_invalida(self):
        ...
        resp = self.client.post(reverse('custo_criar'), {
            'categoria_custo': str(vinc.pk), 'descricao': 'X',
            'data': '2026-06-10', 'valor': '50', 'mes': '6', 'ano': '2026'})
        self.assertEqual(resp.status_code, 302)
    ...
    def test_editar(self):
        custo = Custo.objects.create(estabelecimento=self.est, categoria_custo=self.cat,
                                     descricao='V', data=date(2026, 6, 1), valor=Decimal('100'))
        resp = self.client.post(reverse('custo_editar', args=[custo.pk]), {
            'categoria_custo': str(self.cat.pk), 'descricao': 'Atualizado',
            'data': '2026-06-15', 'valor': '250', 'mes': '6', 'ano': '2026'})
        self.assertEqual(resp.status_code, 302)
    ...
    def test_excluir(self):
        custo = Custo.objects.create(estabelecimento=self.est, categoria_custo=self.cat,
                                     descricao='D', data=date(2026, 6, 1), valor=Decimal('10'))
...
class PerfilViewTests(TestCase):
    def setUp(self):
        ...
        EstabelecimentoUsuario.objects.create(estabelecimento=self.est, usuario=self.user)
        self.client.force_login(self.user)
    ...
    def test_post_seleciona_estabelecimento_valido(self):
        resp = self.client.post(reverse('perfil'), {'estabelecimento_id': str(self.est.pk)})
        self.assertRedirects(resp, reverse('perfil'))
        self.assertEqual(self.client.session['estabelecimento_ativo_id'], str(self.est.pk))
    ...
    def test_post_estabelecimento_invalido(self):
        ...
        resp = self.client.post(reverse('perfil'), {'estabelecimento_id': str(uuid.uuid4())})
        self.assertRedirects(resp, reverse('perfil'))
...
class EditFormRenderTests(AdminLogadoMixin, TestCase):
    ...
    def test_estabelecimento_editar_get(self):
        est = criar_estabelecimento()
        self.assertEqual(self.client.get(reverse('estabelecimento_editar', args=[est.pk])).status_code, 200)

    def test_categoria_editar_get(self):
        cat = CategoriaCusto.objects.create(nome='C')
        self.assertEqual(self.client.get(reverse('categoria_custo_editar', args=[cat.pk])).status_code, 200)

    def test_caracteristica_editar_get(self):
        c = CaracteristicaAtendimento.objects.create(ordem=1, nome='A', pergunta='Q')
        self.assertEqual(self.client.get(reverse('caracteristica_atendimento_editar', args=[c.pk])).status_code, 200)

    def test_opcao_editar_get(self):
        c = CaracteristicaAtendimento.objects.create(ordem=1, nome='A', pergunta='Q')
        o = CaracteristicaAtendimentoOpcao.objects.create(caracteristica_atendimento=c, nome='Box')
        self.assertEqual(self.client.get(reverse('opcao_caracteristica_editar', args=[c.pk, o.pk])).status_code, 200)

    def test_usuario_editar_get(self):
        u = criar_usuario(email='ed@b.com')
        self.assertEqual(self.client.get(reverse('usuario_editar', args=[u.pk])).status_code, 200)
    ...
    def test_acesso_editar_get(self):
        ...
        self.assertEqual(self.client.get(reverse('acesso_editar', args=[v.pk])).status_code, 200)
...
class EditFormRenderTenantTests(AutenticadoComEstabelecimentoMixin, TestCase):
    ...
    def test_cliente_editar_get(self):
        ...
        self.assertEqual(self.client.get(reverse('cliente_editar', args=[c.pk])).status_code, 200)

    def test_atendimento_editar_get_calcula_totais(self):
        at = criar_atendimento(self.est, duracao=125)
        Pagamento.objects.create(atendimento=at, forma_pagamento=None, valor=Decimal('150'))
