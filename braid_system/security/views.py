...
def google_login(request):
    ...
    if not google_oauth_configured():
        messages.error(request, "O login com Google ainda não está configurado.")
...
def google_callback(request):
    ...
    try:
        ...
        messages.error(
            request, "Não foi possível concluir o login com Google. Tente novamente."
        )
        return redirect("home")
    ...
    if not email or not email_verificado:
        messages.error(request, "Sua conta Google não retornou um e-mail verificado.")
        return redirect("home")
    ...
    if not user.ativo:
        ...
        messages.error(request, "Sua conta está inativa. Procure um administrador.")
        return redirect("home")
